...
1. **자동화 파이프라인 (Auto Pipeline)**
   ...
   - **무음 자동 제거 (Silence Cut)**: 0.65초 이상의 죽은 구간만 컷해 밀도를 올린다. 0.4초도 못 줄이는 컷, 컷 사이 1.2초 미만의 조각은 만들지 않는다(의미 없는 점프컷 방지).
   - **컷 인지형 등속도 인물 추적 (Constant Speed Face Tracking)**: YuNet ONNX 기반 얼굴 검출 + 480px/s 등속 선형 트래킹 및 정적 샷 락(Lock)으로 멀미 없는 안정적인 9:16 세로 뷰포트 구성.
   - **영/한 2단 자막 하드번인 + 하이라이트 줌인**: 10~16자 호흡 단위 분절, 영문 번역, 골드 하이라이트 자막 연동 줌인 효과.
   - **초미세 버블팝 SFX 오버레이**: 45ms 미니멀 버블팝(900→1800Hz) 효과음을 하이라이트 타이밍에 자동 믹싱.
   - **포토제닉 프레임 선별 & 2단 훅(Hook) 썸네일 & 영상 인트로 자동 결합**: 3fps 콘택트 시트 기반 표정 선별 + 인스타그램 3:4 릴스 탭/4:5 피드 안전영역(A안) 완벽 대응 2단 훅 타이틀 생성 + 플랫폼 자동 업로드를 위한 영상 첫머리 무음(0.7s) 썸네일 노출 및 블랙 페이드 트랜지션 자동 결합.
